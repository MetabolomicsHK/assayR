...
reconfigure.table <- function(X, column.name.split.pattern="^([,]+),([,]+)", combine.row.names.with.match=1, has.row.name.column=false, FUN=sum, ...){
  m <- melt(X)
  t <- str_match(as.character(m$variable), "(.*)(0|\\.\\d+[CNH]\\.\\d+)")  # matrix of matches
  # join them up.
...
combine.rep.cols <- function(x, FUN=mean, ...){
  ...
  x$X <- rownames(x)
  m <- melt(x)
  m$variable <- sub("n\\d+(.*)-\\d+(0|-\\d+[CNH]-\\d+)", "n99\\1-9\\2", as.character(m$variable))
...
assay.plot = function(q, repeat.inj.pattern = ".*_n[[:digit:]]-(.*)",
...
reshape.assay.result2 = function(r, xpatt, ypatt, fun){
  ...
  if(length(x) == 0){
    ...
    cat(paste("reshape.assay.result2: xpatt {",
              xpatt,
  ...
  if(length(y) == 0){
    ...
    cat(paste("reshape.assay.result2: ypatt {",
              ypatt,"} did not match anything in rownames(r):",
              rn,
              "\n"))
    return()
  }
...
detect.peaks = function(tic, samples=20, wavethresh=2e4, normlim=5){
  ...
  if(length(lp)>0){
    for(peakn in 1:length(lp[,1])){
      peakstart = lp[peakn,1]-1
      peakend = lp[peakn,2]+1
      peaktic = tic
      L = length(peaktic)
...
get.peaks = function(path=".",pattern=".tsv",rt.min=0,rt.max=Inf,seconds=10,threshold=1e7){
  ...
  for(file in files){
    ...
    # smoothing?
    tic$sumIntensity = smooth(tic$sumIntensity)
    tics[[file]] = tic
  }
  setwd(initialpath)
...
run.config.peaks = function(
  ...
  config$N15[is.na(config$N15) | config$N15 == ""] <- 0
  config$H2[is.na(config$H2) | config$H2 == ""] <- 0
  config$interactive[grep("Yes|Inter|Okay|1|True|T",config$interactive, ignore.case=TRUE)] <- "yes"
  print(config)
    ...
    if(Interactive == TRUE && config$interactive[i] == "yes"){
      while(1){
        ...
        if(usersays == "y"){
          ...
          line = readline(paste("enter new value for threshold [",config$threshold[i],"]: "))
          if(nchar(line) > 0){
...
msconvert_all = function(){
  ...
  system('msconvert --mzML --simAsSpectra --filter "peakPicking true 1-" -o mzMLpos --filter "polarity +" *.raw')
  system('msconvert --mzML --simAsSpectra --filter "peakPicking true 1-" -o mzMLneg --filter "polarity -" *.raw')
}
